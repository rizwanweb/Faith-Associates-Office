...
{
    public partial class JobEntryForm : TemplateForm
    {
        ...
        private void SaveJobPayorders()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dgvPayorders.Rows)
                {
                    ...
                    db.InsertOrUpdateRecord("usp_JobPayorderInsertNewPayorder", GetPayorderObjects(pid, particulars, amount, detail));
                }
            }
            catch (Exception ex)
            {
        ...
        private object GetObjects()
        {
            ...
            j.JobID = (this.isUpdate) ? this.JobID : 0;
            j.JobNo = Convert.ToInt32(txtJobNo.Text);
            j.JobDate = dtJob.Value;
        ...
        private bool CheckIfJobExist()
        {
            ...
            {
                DBSQLServer db = new DBSQLServer(AppSetting.ConnectionString());
                bool ifJobExist = Convert.ToBoolean(db.GetScalarValue("usp_JobsCheckJobDetails", GetParameters()));

                if (ifJobExist)
                {
                    RSMessageBox.ShowErrorMessage("Job Already Exist");
                    return true;
                }
                else
                {                    
        ...
        private void btnJobSearch_Click(object sender, EventArgs e)
        {
            this.JobID = 0;
            isUpdate = false;
            SearchJobsForm search = new SearchJobsForm();
            search.ShowDialog();
        ...
        private void LoadPayorderToDGV()
        {
            ...
            DataTable dt = db.GetDataList("usp_PayorderHeaderGetHeaderByJobID", para);
        ...
        private void LoadDataIntoJobEntryFormIfUpdate()
        {
            ...
            para.Parameter = "@JobID";
            para.Value = JobID;
            DataTable dt = db.GetDataList("usp_JobsGetJobByID", para);
            DataRow row = dt.Rows[0];
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                try
                {
                    ...
                    para.Parameter = "@JobID";
                    para.Value = this.JobID;
                    db.DeleteRecord("usp_JobPayordersDeleteByJobID", para);
                    db.DeleteRecord("usp_JobsDeleteJob", para);

                    // TODO : Delete from Job Payorders

                    RSMessageBox.ShowSuccessMessage("Job Deleted Successfully..");
                    ListData.ClearFormControls(this);
                    this.JobID = 0;
                    txtJobNo.Focus();
                }
                ...
                {
                    RSMessageBox.ShowErrorMessage("Problem Deleting Job.. " + ex.Message.ToString());                    
                }               
            }
        }
